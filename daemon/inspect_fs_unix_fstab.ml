...
(* For /dev/disk/by-path there is a limit to what we can do because
 * the PCI topology inside the appliance is different from the
 * original guest.  Like resolve_diskbyid, this heuristic will only
 * work for guests that have a single block device.
 *
 * XXX Use hints from virt-p2v if available.
 * See also: https://bugzilla.redhat.com/show_bug.cgi?id=1608131
 *)
